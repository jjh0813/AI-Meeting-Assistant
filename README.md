...
- 담당자·업무 의미·기한을 함께 검증하고 가장 최근 항목 하나만 제시하는 일정 변경 후보 확인
- 업무 상태 관리와 캘린더형 대시보드
...
동일한 Google 계정을 여러 Noting 계정에서 선택하더라도 각 Noting 계정은 서로 다른 보조 캘린더를 사용합니다. Noting에서 로그아웃하거나 Google 연결을 해제하면 현재 Noting 계정의 OAuth 토큰을 제거해 다음 연결에서 Google 계정 선택 화면을 다시 거치게 합니다. 전용 캘린더 ID와 일정 연결 정보는 보존하므로 같은 Google 계정으로 다시 승인했을 때 기존 전용 캘린더를 재사용합니다. 같은 Google 계정을 사용하는 다른 활성 연결이 있으면 해당 연결의 토큰까지 무효화하지 않습니다. 이전 버전에서 `primary` 캘린더에 연결된 사용자는 로그인 후 첫 동기화에서 전용 캘린더 방식으로 자동 전환되며, 기존 토큰에 생성 권한이 없는 경우에만 `Google Calendar 다시 연결` 안내가 나타납니다.
