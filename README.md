...
- 사용자 검토 후 회의록 수정 및 재분석
- Gemma 기반 회의 요약과 할 일·담당자·기한·요청사항 추출
...
4. 사용자가 마스킹된 회의록을 검토하고 필요한 내용을 수정합니다.
5. Gemma가 요약과 업무 항목을 구조화된 JSON으로 추출합니다.
...
회의록이 수정되면 이전 요약, 업무 항목 및 검색 청크를 제거합니다. 수정된 내용은 다시 분석해야 검색과 보고서에 반영됩니다.
...
현재 저장소는 기본 테이블이 구성된 데이터베이스를 전제로 하며, 완전한 초기 스키마 마이그레이션 체인은 아직 포함하지 않습니다. `scripts/migrate_action_items_rag.py`는 기존 `action_items` 테이블에 상태, 임베딩 및 일정 변경 관계를 추가하는 증분 마이그레이션입니다.
